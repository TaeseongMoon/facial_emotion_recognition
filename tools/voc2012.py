...
def build_example(annotation, class_map):
    img_path = os.path.join(
        FLAGS.data_dir, 'img', annotation['filename'])
...
def main(_argv):
    ...
    # image_list = glob(os.path.join(FLAGS.data_dir,'img/**/*.jpg'), recursive=True)
    image_list = [ x[:-4] for x in os.listdir(os.path.join(FLAGS.data_dir, 'img')) if x.endswith('.jpg')]
